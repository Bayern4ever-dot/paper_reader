...
motivation: 单一视觉语言模型在处理文本、图像或跨模态不匹配导致的复杂虚假信息时能力有限，且现有智能体系统缺乏深度推理路径探索。
method: 提出AgentM3D框架，采用自适应测试时缩放、Best-of-N机制、级联决策链以及根据样本难度动态调整推理路径的规划智能体。
result: 在两个M3D基准测试上的实验表明，AgentM3D在零样本检测性能上优于现有的VLM和智能体基准模型。
conclusion: 通过引入自适应推理缩放和多智能体协同，AgentM3D有效提升了多模态虚假信息检测的准确性与计算效率。
---

## 摘要
视觉语言模型 (VLMs) 已被证明在检测社交平台上的多模态虚假信息方面非常有效，特别是在缺乏标注或标注延迟的零样本设置下。然而，单个 VLM 的能力在更复杂的混合来源多模态虚假信息检测 (M3D) 任务中显得不足。以带有说明文字的图像为例，在 M3D 中，虚假信息可能源于不实文本、伪造图像或两种模态之间的不匹配。尽管最近的智能体系统可以通过连接特定模态的 VLM 智能体来处理零样本 M3D，但其有效性仍受限于其架构。在现有的智能体 M3D 解决方案中，对于任何输入样本，每个智能体仅执行一次前向推理过程，这使得决策在具有挑战性的案例中容易受到模型随机性和推理错误的影响。此外，缺乏对替代推理路径的探索阻碍了现代 VLM 充分利用其推理能力。在这项工作中，我们提出了 AgentM3D，这是一个用于零样本 M3D 的多智能体框架。为了增强 VLM 的推理能力，我们引入了一种自适应测试时扩展范式，其中每个特定模态的 VLM 智能体应用 Best-of-N 机制，并配合评论智能体进行任务对齐评分。这些智能体被组织在一个级联的特定模态决策链中，以减少不必要的计算并限制错误传播。为了确保可扩展性，规划智能体根据样本难度动态确定最大推理路径数量，而自适应停止机制则防止每个智能体内部的过度推理。在两个 M3D 基准测试上的广泛实验表明，与各种基于 VLM 和智能体的基准模型相比，AgentM3D 实现了最先进的零样本检测性能。
