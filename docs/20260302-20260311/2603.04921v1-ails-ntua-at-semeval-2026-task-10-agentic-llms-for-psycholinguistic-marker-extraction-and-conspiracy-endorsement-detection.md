...
motivation: 传统分类器在处理阴谋论时往往将语义推理与结构定位混为一谈，且容易因无法区分客观报道与主观认可而陷入“报告者陷阱”。
method: 采用解耦设计，结合动态判别思维链（DD-CoT）进行标记提取，并利用由对抗性平行委员会和校准法官组成的“反回声室”架构进行检测。
result: "在标记提取和认可检测任务上分别实现了0.24和0.79的Macro F1值，较基准线分别提升了100%和49%。"
conclusion: 该方法通过智能体化设计成功解决了心理语言学特征提取中的脆弱性问题，为构建可解释的阴谋论分析系统提供了有效途径。
---
