...
tldr: 本研究针对多模态遥感语义分割中预训练视觉大模型适配成本高及模态不平衡问题，提出了MoBaNet框架。该框架采用对称双流架构并冻结骨干网络，利用跨模态提示注入适配器实现深度交互，通过差异引导门控融合模块优化特征选择，并引入模态条件随机掩码策略缓解不平衡。实验证明，MoBaNet在保持极低参数量的情况下，在ISPRS数据集上取得了领先的分割性能，实现了高效且鲁棒的多模态融合。
selection_source: fresh_fetch
motivation: 旨在解决预训练视觉大模型在多模态遥感任务中微调开销大以及优化过程中存在的模态不平衡挑战。
method: 提出一种参数高效的对称融合框架MoBaNet，包含跨模态提示注入适配器、差异引导门控融合模块及模态条件随机掩码策略。
result: 在ISPRS Vaihingen和Potsdam基准测试中，该方法以显著减少的可训练参数量实现了最先进的分割精度。
conclusion: MoBaNet通过对称架构与平衡策略的结合，为高效利用预训练大模型进行多模态遥感图像解译提供了有效方案。
---
