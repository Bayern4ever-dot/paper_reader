...
tldr: 本研究针对恶劣天气下的无监督域自适应语义分割，指出传统的静态课程学习易导致类别偏差。为此，提出一种基于强化学习的自主类别调度器，通过高维状态编码器捕捉模型训练动态，并结合类别公平的策略梯度目标来动态调整学习顺序。该方法配合源域-目标域混合监督，使模型能自适应地关注最具信息量的类别，在ACDC、Dark Zurich等多个基准测试中达到领先水平。
selection_source: fresh_fetch
motivation: 传统的课程学习依赖手工设计的静态启发式规则，无法适应模型演进的训练动态，导致严重的类别偏差。
...
conclusion: 通过强化学习实现的动态类别调度能有效平衡各类别学习进度，显著提升复杂环境下的语义分割鲁棒性。
---

## 摘要
语义类别的学习顺序显著影响语义分割的无监督领域自适应，尤其是在恶劣天气条件下。大多数现有的课程学习依赖于手工设计的启发式方法（如固定的不确定性指标）并遵循静态调度，这无法适应模型不断演进的高维训练动态，从而导致类别偏差。受强化学习的启发，我们将课程学习建模为一个序列决策问题，并提出了一种自主类别调度器。该调度器由两个组件组成：(i) 一个高维状态编码器，将模型的训练状态映射到潜空间并提取指示进度的关键特征；(ii) 一个类别公平的策略梯度目标，确保各类别之间的平衡提升。结合源域-目标域混合监督，学习到的类别排名引导网络在每个阶段关注最具信息量的类别，从而实现更具自适应性和动态性的学习。值得注意的是，我们的方法在三个广泛使用的基准测试（如 ACDC、Dark Zurich 和 Nighttime Driving）上达到了最先进的性能，并在从合成到真实的语义分割中展示了泛化能力。
