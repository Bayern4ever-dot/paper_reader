...
title_zh: 无需训练的清晰视觉：缓解遥感多模态大语言模型中的幻觉
...
## 摘要
多模态大语言模型（MLLMs）在遥感视觉问答（RS-VQA）中存在显著的幻觉问题，这主要是由大规模场景中的视觉定位失败或对细粒度小目标的误解引起的。为了系统地分析这些问题，我们引入了 RSHBench，这是一个基于协议的基准测试，用于对事实性幻觉和逻辑性幻觉进行细粒度诊断。为了缓解由定位引起的事实性幻觉，我们进一步提出了相对注意力驱动的主动推理（Relative Attention-Driven Actively Reasoning, RADAR），这是一种无需训练的推理方法，它利用 MLLMs 的内在注意力在测试时引导渐进式定位和细粒度局部推理。在多种 MLLMs 上进行的广泛实验表明，RADAR 持续提升了 RS-VQA 的性能，并减少了事实性和逻辑性幻觉。代码和数据将在以下网址公开：https://github.com/MiliLab/RADAR
