...
tags: ["query:tsds"]
score: 8.0
evidence: 根据局部转移动态将时间序列划分为连续的时间块
...
result: TMTF 能够有效区分并可视化时间序列中的不同动力学状态（如持久性、均值回归等），并揭示了分块处理中的偏差-方差权衡。
conclusion: TMTF 扩展了 MTF 的表征能力，通过引入局部动态特性，为非平稳时间序列的深度学习分析提供了一种更精确的图像化编码方案。
---

## 摘要
由 Wang 和 Oates (2015) 提出的马尔可夫转移场 (MTF) 通过将每对时间步映射到其分位数状态之间的转移概率（由单个全局转移矩阵估计），将时间序列编码为二维图像。当转移动态是平稳的时，这种构造是有效的，但当过程随时间改变状态（regime）时，它会产生误导性的表示：全局矩阵对不同状态进行了平均，导致生成的图像丢失了关于每个动态状态“何时”处于活跃状态的所有信息。在本文中，我们提出了时间马尔可夫转移场 (TMTF)，这是一种扩展方法，它将序列划分为 $K$ 个连续的时间块，为每个块估计一个单独的局部转移矩阵，并组装图像，使得每一行反映其对应块的局部动态，而非全局平均值。生成的 $T \times T$ 图像具有 $K$ 条纹理各异的水平带，每条带都编码了一个时间段的转移动态。我们给出了正式定义，确立了该表示的关键结构属性，通过一个完整的数值示例具体阐述了其与全局 MTF 的区别，分析了时间分块引入的偏差-方差权衡，并根据过程属性（如持久性、均值回归和趋势行为）讨论了局部转移矩阵的几何解释。TMTF 具有振幅无关性和保序性，使其适合作为卷积神经网络的输入通道，应用于时间序列特征刻画任务。
