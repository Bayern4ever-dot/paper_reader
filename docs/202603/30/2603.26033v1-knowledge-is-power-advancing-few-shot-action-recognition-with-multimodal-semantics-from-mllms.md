...
tags: ["query:mmfnd"]
score: 7.0
evidence: 利用多模态大语言模型语义进行动作识别
tldr: 本研究针对少样本动作识别（FSAR）中多模态大模型（MLLM）利用不充分的问题，提出了首个端到端方法FSAR-LLaVA。该方法将Video-LLaVA等模型作为多模态知识库，通过多模态特征增强模块提取并解耦时空语义特征，利用复合任务导向原型构建缓解分布偏移，并引入免训练的多模态原型匹配度量。实验证明，该方法在极少可训练参数下实现了性能的显著提升。
selection_source: fresh_fetch
motivation: 现有的少样本动作识别方法主要将多模态大模型用于生成文本描述，未能充分挖掘其深层的多模态语义知识和联合度量潜力。
method: 提出了FSAR-LLaVA框架，利用MLLM解码器提取并解耦视觉与文本特征，结合任务导向的原型构建和自适应多模态匹配度量进行识别。
result: 在多个主流基准数据集上的实验结果显示，该方法在仅需极少可训练参数的情况下，性能显著优于现有最先进技术。
conclusion: 本研究成功将多模态大模型转化为强大的知识库，通过深度语义增强和多模态联合度量，有效提升了少样本动作识别的泛化能力。
---
