...
tldr: 针对多模态大模型中视觉编码器微调不稳定的问题，本文分析发现其根源在于忽略上下文导致的视觉偏好冲突。为此提出了CoVFT框架，通过上下文向量提取（CVE）和上下文混合专家（CoMoE）模块，将多模态上下文显式引入视觉适配过程。实验表明，CoVFT在12个基准测试中实现了SOTA性能，显著提升了微调稳定性，甚至使7B模型性能超越了13B模型，证明了上下文感知微调的巨大潜力。
selection_source: fresh_fetch
motivation: 现有视觉微调方法因忽略多模态上下文导致参数更新方向冲突，使得微调效果往往难以稳定超越冻结视觉编码器的基准。
method: 提出CoVFT框架，通过集成上下文向量提取和上下文混合专家模块，将多模态信息显式融入视觉编码器的动态适配中。
result: 在12个多模态基准测试中，CoVFT展现了卓越的稳定性和性能，助力7B规模的模型在平均表现上超过了13B模型。
conclusion: 将多模态上下文引入视觉微调过程能有效解决优化冲突，是释放视觉编码器潜力并提升多模态大模型性能的关键路径。
---
