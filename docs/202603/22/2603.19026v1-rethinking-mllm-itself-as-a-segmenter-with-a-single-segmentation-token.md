...
title_zh: 重新思考将 MLLM 自身作为仅需单个分割 Token 的分割器
...
## 摘要
最近利用多模态大语言模型（MLLM）的分割方法在目标级分割和增强空间感知方面表现出了可靠性。然而，几乎所有先前的方法都主要依赖专门的掩码解码器来解释生成的分割相关嵌入和视觉特征中的掩码，或者引入多个额外的 Token 来辅助。本文旨在研究是否以及如何通过单个分割嵌入（SELF1E）从 MLLM 自身解锁分割能力，并在取得竞争性结果的同时消除对外部解码器的需求。为此，我们的方法针对 MLLM 中像素重组（pixel-shuffled）图像特征分辨率降低的根本限制。首先，我们保留原始未压缩分辨率的图像特征，并使用从 MLLM 处理后的压缩特征中提取的残差特征对其进行填充，从而提高特征精度。随后，我们分别对经过和未经 LLM 处理的图像特征集成像素逆重组（pixel-unshuffle）操作，以释放压缩特征的细节并在未压缩分辨率下放大残差特征，从而进一步增强填充特征的分辨率。此外，我们重新设计了具有双重感知路径（即图像到图像和图像到分割）的注意力掩码，实现了像素与分割 Token 之间丰富的特征交互。在多个分割任务上的全面实验验证了 SELF1E 达到了与基于专门掩码解码器的方法相当的性能，证明了在 MLLM 中实现无解码器分割的可行性。项目页面：https://github.com/ANDYZAQ/SELF1E。
