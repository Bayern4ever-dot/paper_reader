...
tldr: 针对多模态大模型在通用嵌入任务中面临的任务冲突问题，本文提出TSEmbed框架。该框架结合MoE与LoRA技术显式解耦冲突的任务目标，并引入专家感知负采样（EANS）策略，利用专家路由分布作为语义相似度代理来识别难负样本。通过两阶段学习范式确保训练稳定，TSEmbed在MMEB及工业数据集上均达到SOTA水平，为多模态嵌入的任务扩展奠定了基础。
selection_source: fresh_fetch
motivation: 多模态大模型在适配为通用嵌入模型时，不同任务目标之间的冲突严重阻碍了其性能提升。
method: 采用MoE与LoRA结合的架构解耦任务，并利用专家路由分布进行动态负采样及两阶段训练。
result: 在MMEB基准测试和真实工业数据集上均取得了最先进的性能，证明了该框架的有效性。
