...
title_zh: T2SGrid：面向视频时序定位的时序到空间网格化
...
tldr: 针对视频时序定位（VTG）中现有大模型在处理时序动态时的计算开销大、空间细节丢失等问题，本文提出T2SGrid框架。该方法通过重叠滑动窗口将视频切片，并将每组帧按行优先顺序排列成复合网格图像，将时序理解转化为空间布局理解。这种“时空网格化”不仅保留了时序信息，还增强了局部注意力，结合复合文本时间戳实现了高效且精准的视频定位，在多个基准测试中表现优异。
selection_source: fresh_fetch
motivation: 现有的视觉大模型在处理视频时序信息时存在计算开销大、注意力稀疏或空间细节受损等局限性。
method: 提出将视频帧按时序排列成2D网格图像的“时空网格化”方案，并结合重叠滑动窗口和复合文本时间戳进行建模。
result: 在标准视频时序定位（VTG）基准测试中，T2SGrid 取得了超越现有方法的卓越性能。
conclusion: 将时序序列转化为结构化空间布局是一种增强视频大模型时序感知能力的有效且高效的途径。
---

## 摘要
视频时序定位（VTG）旨在定位与自然语言查询相对应的视频片段，这需要对复杂的时序动态有全面的理解。现有的视觉大语言模型（Vision-LMMs）通常通过位置编码、基于文本的时间戳或视觉帧编号来感知时序动态。然而，这些方法存在显著的局限性：为每一帧分配基于文本的时间戳标记会引入额外的计算开销并导致视觉注意力的稀疏性，位置编码难以捕捉绝对时序信息，而视觉帧编号往往会损害空间细节。为了解决这些问题，我们提出了时序到空间网格化（T2SGrid），这是一个将视频时序理解重新表述为空间理解任务的新型框架。T2SGrid 的核心思想是以视频片段而非单个帧为单位处理视频内容。我们采用重叠滑动窗口机制将视频分割成时序片段。在每个窗口内，各帧按时间顺序以行优先顺序排列成一张复合网格图像，从而有效地将时序序列转换为结构化的二维布局。网格化不仅编码了时序信息，还增强了每个网格内的局部注意力。此外，T2SGrid 允许使用复合文本时间戳来建立全局时序感知。在标准 VTG 基准测试上的实验表明，T2SGrid 取得了优异的性能。
