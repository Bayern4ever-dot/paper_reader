...
title_zh: GALACTIC：时间序列聚类的全局与局部不可知反事实解释
...
tldr: 针对时间序列聚类缺乏可解释性的问题，本文提出GALACTIC框架，首次将反事实解释引入无监督聚类。该框架在局部层面通过聚类感知优化生成扰动，揭示实例跨越聚类边界的最小变化；在全局层面利用最小描述长度（MDL）准则提取非冗余的代表性解释摘要。实验证明，该方法在生成稀疏局部解释和简洁全局摘要方面显著优于现有基准，为理解聚类动态提供了新视角。
selection_source: fresh_fetch
motivation: 现有的时间序列聚类解释方法主要依赖特征归因，难以识别实例在不同聚类边界间转换的关键动态特征。
method: 提出一种统一框架，局部通过聚类感知优化生成扰动，全局利用基于MDL的次模函数优化算法筛选代表性反事实解释。
result: 在UCR数据集上的评估显示，GALACTIC能产生更稀疏的局部反事实解释，并提供比现有方法更简洁的全局解释摘要。
conclusion: GALACTIC作为首个结合局部与全局反事实解释的聚类框架，为无监督时间序列模式发现提供了高效且具有理论保证的可解释性方案。
---

## 摘要
时间序列聚类是模式发现的基础工具，然而现有的可解释性方法（主要基于特征归因或元数据）无法识别导致实例跨越聚类边界的转换。虽然反事实解释（CEs）能够识别改变模型预测所需的最小时间扰动，但它们大多局限于监督学习场景。本文介绍了 GALACTIC，这是第一个为无监督时间序列聚类桥接局部和全局反事实可解释性的统一框架。在实例层面（局部），GALACTIC 通过一种感知聚类的优化目标生成扰动，该目标遵循目标聚类和底层聚类分配。在聚类层面（全局），为了减轻认知负荷并增强可解释性，我们将问题表述为代表性反事实解释的选择问题。我们提出了一种最小描述长度（MDL）目标，用于提取表征聚类间转换的非冗余全局解释摘要。我们证明了我们的 MDL 目标是超模的（supermodular），这使得相应的 MDL 缩减可以被构建为一个单调子模集合函数。这使得一种具有可证明的 (1-1/e) 近似保证的高效贪婪选择算法成为可能。在 UCR 存档上的广泛实验评估表明，与针对该问题调整的最先进基准方法相比，GALACTIC 产生了显著更稀疏的局部反事实解释和更简洁的全局摘要，为通过反事实解释来解读聚类时间序列提供了第一种统一的方法。
