...
tags: ["query:mmfnd"]
score: 7.0
evidence: 抑郁症的多模态检测及模态间不一致性处理
tldr: 针对抑郁症诊断中多模态信息不一致、无关干扰及个体差异等挑战，本文提出了一种个体感知的多模态抑郁相关表征学习框架（IDRL）。该框架通过解耦多模态表征以提取共性与特性抑郁信号并抑制无关信息，同时引入个体感知融合模块动态调整特征权重，实现了更稳健、精准的抑郁症自动检测。
selection_source: fresh_fetch
motivation: 现有多模态抑郁检测方法难以处理模态间冲突、无关信息干扰以及不同个体在抑郁表现上的显著差异。
method: 提出IDRL框架，通过解耦学习提取模态共性、特性及无关表征，并利用个体感知融合模块动态调整特征权重。
result: 在广泛的实验中，IDRL在多模态抑郁检测任务上展现出了优于现有方法的性能和更强的鲁棒性。
conclusion: 该研究证明了通过解耦表征和个体感知融合能有效提升多模态抑郁诊断的准确性，为个性化心理健康监测提供了新思路。
---

## 摘要
抑郁症是一种严重的精神障碍，可靠的识别在早期干预和治疗中起着至关重要的作用。多模态抑郁检测旨在通过联合建模来自多个模态的互补信息来提高诊断性能。近年来，许多多模态学习方法被提出用于抑郁分析；然而，这些方法存在以下局限性：1）模态间的不一致性和抑郁无关的干扰，即抑郁相关线索可能在不同模态间发生冲突，而大量无关内容掩盖了关键的抑郁信号；2）多样化的个体抑郁表现，导致模态和线索重要性存在个体差异，从而阻碍了可靠的融合。为了解决这些问题，我们提出了一种用于鲁棒抑郁诊断的个体感知多模态抑郁相关表征学习框架（IDRL）。具体而言，IDRL 1）将多模态表征解耦为模态共有抑郁空间、模态特定抑郁空间和抑郁无关空间，以在抑制无关信息的同时增强模态对齐；2）引入了个体感知模态融合模块（IAF），根据解耦后的抑郁相关特征的预测显著性动态调整其权重，从而实现针对不同个体的自适应跨模态融合。大量实验表明，IDRL 在多模态抑郁检测方面实现了卓越且鲁棒的性能。
