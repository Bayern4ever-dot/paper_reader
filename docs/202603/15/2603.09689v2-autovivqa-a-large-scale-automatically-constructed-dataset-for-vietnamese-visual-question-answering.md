...
tags: ["query:mmfnd"]
score: 6.0
evidence: 多模态融合以及视觉和文本信息的联合理解
tldr: 本研究针对低资源语言越南语的视觉问答（VQA）任务，探讨了基于Transformer架构的多模态融合方法。通过利用PhoBERT和ViT等预训练模型，研究者构建并分析了大规模自动生成的AutoViVQA数据集。本文系统比较了多种自动评估指标在多语言环境下的表现，旨在提升越南语VQA系统的视觉对齐能力与评估准确性，为低资源多模态学习提供了重要资源。
selection_source: fresh_fetch
motivation: 旨在解决越南语视觉问答领域数据集匮乏以及现有自动评估指标与人类判断一致性不足的问题。
method: 采用基于Transformer的架构，结合PhoBERT文本预训练和ViT视觉预训练模型进行多模态特征融合与实验。
result: 构建了大规模自动生成的AutoViVQA数据集，并系统对比了BLEU、CIDEr等指标在多语言设置下的评估效果。
conclusion: 利用预训练Transformer模型能有效提升越南语VQA性能，且大语言模型有助于改善自动评估指标的准确性。
---

## 摘要
视觉问答（VQA）是一项基础的多模态任务，要求模型共同理解视觉和文本信息。早期的 VQA 系统严重依赖语言偏见，这促使后续工作开始强调视觉定位（visual grounding）和平衡数据集。随着文本和视觉领域大规模预训练 Transformer 的成功——例如用于越南语理解的 PhoBERT 和用于图像表示学习的 Vision Transformers (ViT)——多模态融合取得了显著进展。针对越南语 VQA，已经引入了多个数据集以促进低资源多模态学习的研究，包括 ViVQA、OpenViVQA 以及最近提出的 ViTextVQA。这些资源使得在越南语语境下集成语言和视觉特征的模型基准测试成为可能。VQA 系统的评估通常采用最初为图像描述或机器翻译设计的自动指标，如 BLEU、METEOR、CIDEr、召回率（Recall）、精确率（Precision）和 F1 分数。然而，最近的研究表明，大型语言模型可以进一步提高 VQA 任务中自动评估与人类判断之间的一致性。在这项工作中，我们利用基于 Transformer 的架构探索越南语视觉问答，同时利用文本和视觉预训练，并系统地比较了多语言设置下的自动评估指标。
