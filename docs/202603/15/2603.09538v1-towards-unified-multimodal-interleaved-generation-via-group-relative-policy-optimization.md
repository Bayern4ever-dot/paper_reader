...
score: 6.0
evidence: 多模态交错生成与理解
tldr: 针对统一视觉语言模型在图文交错生成能力的不足，本文提出一种基于强化学习的后训练策略。该方法利用扩展的组相对策略优化（GRPO）框架，结合涵盖文本相关性、图文对齐和结构保真度的混合奖励及过程级奖励，在无需大规模交错数据集的情况下，显著提升了模型在视觉叙事等复杂任务中的生成质量与连贯性。
selection_source: fresh_fetch
motivation: 现有统一视觉语言模型难以生成高质量的图文交错内容，且受限于缺乏大规模标注的交错数据集。
method: 提出一种基于GRPO的强化学习框架，通过混合奖励和过程级奖励在单一解码轨迹中联合优化文本与图像生成。
result: 在MMIE和InterleavedBench基准测试中，该方法显著提高了多模态交错生成的质量、连贯性和结构准确性。
conclusion: 研究证明了通过策略优化和混合奖励机制，可以有效激发预训练模型在复杂多模态交错任务中的潜力。
---
