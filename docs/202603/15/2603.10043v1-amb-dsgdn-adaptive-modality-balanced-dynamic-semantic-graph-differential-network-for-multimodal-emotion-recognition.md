...
motivation: 现有方法难以有效过滤多模态特征中的冗余噪声，且主导模态往往会抑制非主导模态的互补贡献。
method: 提出一种结合差分图注意力机制和自适应模态平衡机制的动态语义图差分网络。
result: 该方法通过显式对比注意力分布消除了共享噪声，并利用自适应丢弃概率平衡了各模态在情感建模中的作用。
conclusion: AMB-DSGDN通过精细化的噪声过滤和模态平衡策略，显著增强了多模态情感表示的判别力和鲁棒性。
---
