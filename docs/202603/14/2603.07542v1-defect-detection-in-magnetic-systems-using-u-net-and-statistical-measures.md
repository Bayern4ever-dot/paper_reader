...
motivation: 旨在解决热波动和实验噪声导致磁成像中材料缺陷难以识别的挑战。
method: 结合微磁模拟与U-Net模型，利用时间平均值、标准差和潜在熵等统计指标作为输入进行缺陷分割。
result: 实验表明最优描述符的选择受噪声水平影响，且模型性能极大程度上取决于训练数据与实际噪声统计的匹配度。
conclusion: 该研究为开发抗噪声的磁性系统缺陷检测流程提供了理论依据和实践指南。
---

## 摘要
局部材料的不均匀性会强烈影响磁化动力学和宏观磁性能，然而，当热涨落和实验噪声掩盖了静态对比度时，从磁成像数据中检测此类缺陷仍然具有挑战性。在此，我们研究了强波动磁化状态下的缺陷检测，在这种状态下，不均匀性的特征在时间分辨测量中很大程度上被平均掉了。利用具有随机分布缺陷且材料参数代表 \ce{Ni80Fe20} 的有限温度微磁模拟，我们计算了逐像素的时间平均值、时间标准差和潜在熵，并将其作为基于 U-Net 的语义分割模型的输入。我们发现，最有效的描述符取决于噪声水平，且重要的是，鲁棒检测需要反映预期噪声统计特性的训练数据。这些结果为设计磁成像中具有噪声鲁棒性的缺陷检测工作流提供了实际指导。
