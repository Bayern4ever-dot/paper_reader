...
tldr: 针对无需训练的开放词汇语义分割（OVSS）中跨模态几何信息利用不足及模型复杂度高的问题，本文提出PEARL方法。该方法遵循“先对齐后传播”原则，通过Procrustes对齐在自注意力层实现正交投影，并利用文本感知的拉普拉斯传播优化分割结果。PEARL无需额外训练或辅助骨干网络，在保持低延迟的同时，在多个基准测试中达到了新的SOTA水平。
selection_source: fresh_fetch
motivation: 现有的无需训练OVSS方法往往忽略了跨模态几何对齐，或依赖复杂的辅助模型，导致效率和性能受限。
method: 提出一种包含Procrustes对齐（旋转键向量至查询子空间）和文本感知拉普拉斯传播（基于置信度和图像梯度的图优化）的两阶段推理框架。
result: 在标准基准测试中，PEARL在不使用额外数据或辅助网络的情况下，在有/无背景协议下均取得了领先的性能。
conclusion: PEARL证明了通过简单的几何对齐和精细的传播机制，可以在不增加训练成本的前提下显著提升开放词汇语义分割的质量。
---
