...
motivation: 针对低功耗物联网设备，探讨如何在极度受限的硬件资源下实现高效且实时的时序数据分类。
method: 在五个基准数据集上，对LSTM和1D-CNN模型在MCU上的内存占用、存储消耗及推理速度进行了硬件感知的对比实验。
result: "1D-CNN在准确率上优于LSTM（95%对89%），且节省了35%的RAM和25%的Flash，推理速度从2038毫秒大幅缩减至27.6毫秒。"
conclusion: 1D-CNN凭借其轻量化和高效性，成为可穿戴设备等低功耗TinyML应用中替代LSTM的更优方案。
---
