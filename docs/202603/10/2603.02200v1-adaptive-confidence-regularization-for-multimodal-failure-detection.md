...
motivation: 多模态模型在失效时常出现置信度低于单模态分支的“置信度退化”现象，亟需有效的失效检测机制以确保高风险领域的应用安全。
method: 提出了ACR框架，通过自适应置信度损失惩罚置信度退化，并利用多模态特征交换技术合成具有挑战性的失效样本进行训练。
result: 在四个数据集和三种模态上的广泛实验证明，ACR在多种评估设置下均实现了持续且稳健的性能提升。
conclusion: ACR通过正则化置信度分布和合成失效感知样本，显著增强了多模态模型识别并拒绝不确定预测的能力，提升了系统整体可靠性。
---
