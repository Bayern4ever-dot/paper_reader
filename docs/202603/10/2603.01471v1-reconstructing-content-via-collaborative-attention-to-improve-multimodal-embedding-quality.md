...
score: 6.0
evidence: 提高多模态嵌入质量
tldr: 针对多模态大模型（MLLM）因果注意力机制不利于形成全局紧凑表征的问题，本文提出CoCoA预训练范式。该方法通过重构注意力流并引入基于EOS标记的内容重构任务，强制模型将输入的语义信息压缩至EOS嵌入中，为后续对比学习提供高质量表征。在MMEB-V1基准测试上，基于Qwen2-VL的实验证明CoCoA显著提升了多模态嵌入质量，有效增强了模型在检索和分类任务中的表现。
selection_source: fresh_fetch
motivation: 多模态大模型的因果注意力和下文预测范式主要为生成设计，缺乏对形成全局紧凑表征的显式鼓励，限制了其作为嵌入骨干的效果。
method: 提出CoCoA范式，通过重组注意力流并引入EOS重构任务，驱动模型从EOS嵌入中重建输入内容以实现语义压缩。
result: 在MMEB-V1基准上的实验表明，CoCoA显著提升了基于Qwen2-VL和Qwen2.5-VL模型的嵌入质量。
conclusion: 内容重构是最大化现有数据价值、提升多模态嵌入模型生成紧凑且信息丰富表征能力的有效策略。
---
