...
tags: ["query:tsds"]
score: 7.0
evidence: 具有时空建模和时间一致性的视频语义分割
tldr: 针对状态空间模型（SSM）在视频语义分割中因固定状态空间导致细节信息丢失的问题，本文提出RS-SSM模型。该模型通过通道振幅感知器（CwAP）提取并对齐特定信息的分布特征，并利用遗忘门信息精炼器（FGIR）自适应地反转并精炼遗忘门矩阵，从而补偿压缩过程中丢失的时空细节。实验证明，RS-SSM在四个主流基准测试上达到了SOTA性能，且保持了极高的计算效率。
selection_source: fresh_fetch
motivation: 状态空间模型在压缩过程中会不可避免地遗忘特定的时空细节，限制了其在像素级视频语义分割中的表现。
method: 提出RS-SSM，通过CwAP提取特征分布并利用FGIR反转遗忘门，从而互补性地精炼被遗忘的特定信息。
result: 在四个视频语义分割基准数据集上的实验表明，该方法在保持高效计算的同时实现了最先进的性能。
conclusion: RS-SSM通过精炼被遗忘的细节信息，显著增强了状态空间模型在时空像素级分割任务中的建模能力。
---
