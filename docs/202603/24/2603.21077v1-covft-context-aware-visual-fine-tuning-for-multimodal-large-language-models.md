...
result: 在12个多模态基准测试中取得SOTA性能，且微调后的7B模型在平均表现上超越了13B规模的同类模型。
conclusion: 显式引入上下文感知的视觉微调能有效解决优化冲突，是提升多模态大模型视觉感知与推理能力的有效途径。
---

## 摘要
多模态大语言模型（MLLMs）在跨模态感知和推理方面取得了显著进展，但一个基本问题仍未解决：视觉编码器应该被微调还是冻结？尽管 LLaVA 和 Qwen-VL 等模型取得了成功，但设计选择的不一致和训练设置的异构性阻碍了对 MLLMs 中视觉微调（VFT）的统一理解。通过配置对齐的基准测试，我们发现现有的 VFT 方法在各种多模态任务中无法始终优于冻结基准。我们的分析表明，这种不稳定性源于视觉偏好冲突，即视觉编码器的上下文无关特性在多样化的多模态上下文下引发了发散的参数更新。为了解决这一问题，我们提出了上下文感知视觉微调（CoVFT）框架，该框架显式地将多模态上下文纳入视觉适配中。通过集成上下文向量提取（CVE）和上下文混合专家（CoMoE）模块，CoVFT 分解了冲突的优化信号，并实现了稳定且上下文敏感的视觉更新。在 12 个多模态基准测试上的广泛实验表明，CoVFT 以卓越的稳定性实现了最先进的性能。值得注意的是，使用 CoVFT 微调的 7B MLLM 超过了其 13B 对应模型的平均性能，揭示了 MLLMs 中视觉编码器优化方面巨大的未开发潜力。
