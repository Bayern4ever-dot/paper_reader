...
tldr: 针对传统时间序列异常检测仅依赖数值数据的局限，本文提出MindTS模型，旨在利用多模态信息提升检测性能。该模型通过细粒度时空语义对齐机制整合内外部文本信息，并利用内容压缩重建技术过滤冗余信息，实现了高效的跨模态交互。在六个真实数据集上的实验证明了其在处理异构多模态数据方面的优越性，为复杂动态系统的异常监测提供了新思路。
selection_source: fresh_fetch
motivation: 传统方法主要依赖单一数值模态，难以利用其他模态的补充信息，且面临异构数据语义对齐和信息冗余的挑战。
method: 提出MindTS模型，通过细粒度时空语义对齐整合文本与时间序列，并利用内容压缩重建模块过滤冗余信息以增强跨模态交互。
result: 在六个真实世界多模态数据集上的实验结果显示，MindTS的性能优于或等同于现有的先进异常检测方法。
conclusion: 通过有效的语义对齐和信息压缩，多模态数据融合能显著增强时间序列异常检测的准确性和鲁棒性。
---
