...
tldr: 本研究针对非受限视频中矛盾与犹豫状态识别的挑战，提出了一个整合场景、面部、音频和文本的四模态融合框架。该方法利用VideoMAE捕捉场景动态，结合EmotionWav2Vec2.0与Mamba处理音频，并利用微调的Transformer提取文本特征。通过原型增强的多模态融合策略，该方案在第10届ABAW竞赛中表现出色，显著提升了识别性能，证明了多模态互补信息在复杂行为分析中的重要性。
selection_source: fresh_fetch
motivation: 旨在解决非受限视频中矛盾与犹豫状态因其微妙性、多模态性和上下文依赖性而难以准确识别的问题。
method: 提出了一种融合场景、面部、音频（基于Mamba编码）和文本特征的多模态框架，并采用了原型增强的融合模型集成策略。
result: "多模态融合模型在验证集上取得了83.25%的平均MF1分数，远超单模态基准，并在最终测试中达到了71.43%的性能。"
conclusion: 实验结果强调了整合互补的多模态线索以及采用鲁棒的融合策略在识别复杂心理行为状态中的关键作用。
---
