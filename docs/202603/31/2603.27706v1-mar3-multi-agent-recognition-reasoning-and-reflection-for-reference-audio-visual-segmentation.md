...
## 摘要
引用型音视频分割（Ref-AVS）旨在根据引用表达式中的多模态线索，对有声视频中的目标进行分割。以往的方法忽视了对表达式难度和多模态线索中主导模态的显式识别，过度依赖指令微调数据集的质量进行目标推理，且缺乏对分割结果的反思性验证，从而导致错误的掩码预测。为了解决这些问题，本文提出了一种新型的无需训练的多智能体识别、推理与反思框架，称为 MAR3，以实现高质量的引用型音视频分割。该框架结合了社会学德尔菲（Delphi）理论以实现鲁棒分析，提出了一种共识多模态识别机制，使大语言模型（LLM）智能体能够显式识别引用表达式的难度和多模态线索的主导模态。基于模态主导难度规则，我们提出了一种自适应协作目标推理策略，以可靠地推理所引用的目标。为了进一步确保精确的掩码预测，我们开发了一种反思性学习分割机制，其中检查智能体检查中间分割结果，并迭代修正分割智能体的目标文本提示。实验表明，MAR3 在 Ref-AVSBench 数据集上取得了卓越的性能（J&F 为 69.2%），绝对值超过现有最先进技术（SOTA）3.4%。
