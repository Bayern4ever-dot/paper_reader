...
motivation: 传统方法将预训练生成模型视为特征提取器，这种间接适配方式存在表示失配和流程复杂的问题。
method: 提出一种直接生成掩码的 DiT 适配方案，并设计了针对二值掩码特性的时间步采样策略以实现联合训练。
result: GenMask 在指代分割和推理分割基准上取得了 SOTA 性能，且无需复杂的特征提取管道。
conclusion: 证明了通过直接生成方式适配 DiT 进行分割任务比间接特征提取更具优势且架构更简洁。
---

## 摘要
最近的分割方法利用预训练生成模型作为特征提取器，通过间接特征检索将分割视为下游适配任务。这种隐式使用方式存在表示上的根本不匹配。它还严重依赖间接特征提取流水线，这增加了工作流的复杂性并限制了适配能力。在本文中，我们认为分割任务不应进行间接适配，而应直接以生成方式进行训练。我们发现了这种统一表述的一个关键障碍：二值掩码的 VAE 潜变量分布尖锐、具有噪声鲁棒性且线性可分，这与自然图像的潜变量截然不同。为了弥补这一差距，我们为二值掩码引入了时间步采样策略，该策略强调分割任务的极端噪声水平和图像生成的适度噪声水平，从而实现和谐的联合训练。我们提出了 GenMask，这是一种在原始生成目标下训练以生成黑白分割掩码以及 RGB 空间彩色图像的 DiT。GenMask 保留了原始 DiT 架构，同时消除了对专门为分割任务定制的特征提取流水线的需求。实验表明，GenMask 在指代分割和推理分割基准测试中达到了最先进的性能，消融实验量化了各组件的贡献。
