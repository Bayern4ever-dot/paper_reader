...
## 摘要
传统的多模态方法通常假设模态质量是静态的，这限制了它们在动态现实场景中的适应性。因此，研究者提出了动态多模态方法来评估模态质量并相应地调整其贡献。然而，这些方法通常依赖于经验指标，在噪声水平极低或极高时无法准确衡量模态质量。此外，现有方法通常假设每个模态的初始贡献是相同的，忽略了内在的模态依赖偏差。结果，难以学习的模态会受到双重惩罚，导致动态融合的性能可能劣于静态融合。为了应对这些挑战，我们提出了无偏动态多模态学习（UDML）框架。具体而言，我们引入了一种噪声感知不确定性估计器，该估计器向模态数据添加受控噪声，并根据模态特征预测其强度。这迫使模型学习特征损坏与噪声水平之间的清晰对应关系，从而在低噪声和高噪声条件下都能实现准确的不确定性度量。此外，我们通过模态丢弃（modality dropout）量化了多模态网络中固有的模态依赖偏差，并将其纳入加权机制。这消除了对难学模态的双重抑制效应。在多种多模态基准任务上的广泛实验验证了所提 UDML 的有效性、通用性和泛化能力。代码可在 https://github.com/shicaiwei123/UDML 获取。
