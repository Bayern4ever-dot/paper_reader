...
## 摘要
大语言模型（LLMs）已达到前所未有的流畅度，但仍易受“幻觉”影响，即生成事实错误或无根据的内容。这一局限性在可靠性至关重要的高风险领域尤为关键。我们提出了一种领域锚定的分层检索与验证架构，旨在通过将 LLM 从随机模式匹配器转变为经过验证的真理寻求者，系统地拦截事实错误。该框架利用通过 LangGraph 实现的四阶段自调节流水线：(I) 带有早停（Early-Exit）逻辑的内在验证以优化计算；(II) 利用领域检测器针对特定学科档案的自适应搜索路由；(III) 纠正性文档分级（CRAG）以过滤无关上下文；(IV) 外在生成及随后的原子级断言验证。该系统在来自五个不同基准测试的 650 个查询中进行了评估：TimeQA v2、FreshQA v2、HaluEval General、MMLU Global Facts 和 TruthfulQA。实证结果表明，该流水线在所有环境下均一致优于零样本（zero-shot）基准。胜率在 TimeQA v2 中达到 83.7% 的峰值，在 MMLU Global Facts 中达到 78.0%，证实了其在需要细粒度时间与数值精度的领域中的高效性。在事实回答行中，可靠性（Groundedness）得分稳健地保持在 78.8% 至 86.4% 之间。虽然该架构为错误信息提供了稳健的故障保护，但仍识别出一种持续存在的失败模式，即“虚假前提过度声明”。这些发现提供了多阶段 RAG 行为的详细实证特征，并建议未来的工作应优先考虑检索前的“可回答性”节点，以进一步弥合对话式人工智能中的可靠性差距。
