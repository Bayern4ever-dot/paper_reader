...
title_zh: 质量胜过点击：内在质量驱动的迭代强化学习用于冷启动电商查询建议
...
## 摘要
现有的对话系统依赖查询建议（QS）来增强用户参与度。近期的研究通常结合大语言模型与点击率（CTR）模型，但由于过度依赖海量线上点击数据来进行有效的 CTR 模型训练，在冷启动场景下表现不佳。为填补这一空白，我们提出了 Cold-EQS，这是一种用于冷启动电商查询建议（EQS）的迭代强化学习框架。具体而言，我们利用可回答性、事实性和信息增益作为奖励，以持续优化建议查询的质量。为了持续优化我们的 QS 模型，我们对成组的候选建议查询进行不确定性估计，从而从缺乏点击信号的线上用户查询中筛选出困难且模糊的样本。此外，我们提供了一个包含 16,949 条线上用户查询的 EQS 基准测试集（EQS-Benchmark），用于离线训练和评估。广泛的离线和线上实验一致表明，线上和离线效果之间存在强正相关性。离线和线上实验结果均证明了 Cold-EQS 的优越性，在线上 chatUV 指标上实现了 +6.81% 的显著提升。
