...
tldr: 针对冠状动脉分割中细长管状结构捕捉难和计算开销大的挑战，本文提出MDSVM-UNet框架。该方法在编码阶段引入多向蛇形卷积（MDSConv）以融合三维解剖平面的多视角特征，在解码阶段利用残差视觉Mamba（RVM）实现线性复杂度的远程依赖建模。配合两阶段分割策略，该框架能有效提升复杂血管结构的分割精度并降低计算成本。
selection_source: fresh_fetch
motivation: 传统CNN难以捕捉血管的远程依赖，而ViT计算开销巨大，且冠状动脉细长多分支的特性导致分割极具挑战性。
method: 提出一种结合多向蛇形卷积与残差视觉Mamba的两阶段框架，通过多视角特征融合与状态空间模型优化特征提取与解码。
result: 该方法成功捕捉了冠状动脉的细长扭曲几何特征，在保证计算效率的同时显著提升了分割细节并减少了误报。
conclusion: MDSVM-UNet通过协同可变形卷积与Mamba架构，为资源受限临床环境下的精准冠状动脉分割提供了高效的解决方案。
---
