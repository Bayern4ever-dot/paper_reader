...
tldr: 针对视频推理分割中传统方法在复杂动态场景下轨迹感知能力不足的问题，本文提出了TrajSeg框架。该框架基于多模态大模型（MLLM），引入了双向文本-轨迹对齐机制，通过定位和描述任务增强模型对物体轨迹的理解。结合帧级内容集成模块和统一掩码解码器，实现了高效的端到端训练。实验证明，TrajSeg在多个视频分割基准测试中均取得了SOTA性能，显著提升了视频推理分割的准确性。
selection_source: fresh_fetch
motivation: 现有方法依赖单向且隐式的文本-轨迹对齐，在面对剧烈视频动态时难以准确感知物体轨迹。
method: 引入双向文本-轨迹对齐机制，并利用帧级内容集成模块和统一掩码解码器实现简化的端到端视频分割。
result: TrajSeg在所有引用和推理视频分割数据集的各项指标上均超越了现有最先进的方法。
conclusion: 通过强化轨迹感知和统一架构设计，TrajSeg为解决复杂动态下的视频推理分割任务提供了一个高效且强大的基准模型。
---
